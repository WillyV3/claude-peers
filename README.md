...
Daemons have EDR-aware triage gates. `pr-helper` and `sync-janitor` refuse to run when the local machine is quarantined; `pr-helper` also skips when no open PRs exist across orgs, and `sync-janitor` also skips when no conflict files exist. `fleet-scout` and `librarian` always run but escalate their output when machines are unhealthy. `fleet-memory` skips when there are no active peers and no recent broker events. `fleet-digest` and `llm-watchdog` are unconditional pass-throughs. See [Daemon Guide](docs/DAEMON-GUIDE.md#edr-aware-triage) for full per-daemon triage behavior.
